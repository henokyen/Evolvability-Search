...
namespace NEAT {
	...
	public:
		...
		void serialize(Archive & ar, const unsigned int version)
		{
			...
			//ar & BOOST_SERIALIZATION_NVP(parent1);
			//ar & BOOST_SERIALIZATION_NVP(parent2);
			//ar & BOOST_SERIALIZATION_NVP(struct_change);
			//ar & BOOST_SERIALIZATION_NVP(production);
			//ar & BOOST_SERIALIZATION_NVP(production_count);
		...
		bool mutate_add_node(std::vector<Innovation*> &innovs,int &curnode_id,double &curinnov, bool isevo= false);

		// Mutate the genome by adding a new link between 2 random NNodes 
		bool mutate_add_link(std::vector<Innovation*> &innovs,double &curinnov,int tries, bool isevo= false);
