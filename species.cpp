	...
	if ((randfloat()<NEAT::mutate_only_prob)||
		poolsize == 0) {
			...
			//various mutations
                        ...
                        if(false) { //NEAT::mut_control) {
                         NEAT::trait_param_mut_prob = new_genome->traits[0]->params[2];
                         NEAT::trait_mutation_power = new_genome->traits[0]->params[3];
                         NEAT::mutate_random_trait_prob = new_genome->traits[0]->params[4];
                         NEAT::mutate_link_trait_prob = new_genome->traits[0]->params[5];
                        }
