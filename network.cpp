...
Network::Network(std::vector<NNode*> in,std::vector<NNode*> out,std::vector<NNode*> all,int netid) {
  ...
  outputs=out;
  all_nodes=all;
  name="";   //Defaults to no name  ..NOTE: TRYING TO PRINT AN EMPTY NAME CAN CAUSE A CRASH
  numnodes=-1;
  numlinks=-1;
  net_id=netid;
  adaptable=false;
...
void Network::give_name(char *newname) {
	...
	//strcpy(temp,newname); //henok hoping it is not called any where in the programmer
