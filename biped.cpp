...
dReal evaluate_controller(Controller* controller,noveltyitem* ni,data_record* record,bool log, bool off)
{
    ...
    while (!creatures[0]->abort() && timestep<simtime)
    {
        simulationStep();
        timestep++;
        if (timestep%100 == 0 && novelty_function % 2 == 1)//henok was timestep%100 == 0 && novelty_function % 2 == 1
        {
...
void  biped_evaluate_paralell(NEAT::Organism* org,data_record* data)
{
    noveltyitem *new_item = new noveltyitem;
    //Organism* new_org = new Organism(org, false);
    ...
    new_item->fitness=evaluate_controller(cont, new_item,data,false, false);
    //org->fitness=evaluate_controller(cont, org->noveltypoint,data,false, true);
    org->noveltypoint=new_item;
    org->fitness=new_item->fitness;
   /* if (new_item->fitness < 2.5)
    	new_item->viable=false;
    else new_item->viable=true;*/
    //delete new_item;
    delete cont;
    //std::cout<<"I am org printing my fitness inside Evo class: "<<org->fitness<<std::endl;
...
int biped_novelty_realtime_loop(Population *pop,bool novelty, bool evo) {
...
//      NEAT::compat_threshold = 1000000.0;
        ...
        //to quantify evolvabilityNovelty Search organisms
        if (NEAT::evolvabilitytest && offspring_count % evolveupdate ==0 && novelty) {
            char fn[100];
            sprintf(fn,"%s_evolvability%d.dat",output_dir,offspring_count/evolveupdate);
            for (curorg = (pop->organisms).begin(); curorg != pop->organisms.end(); ++curorg) {
                evolvability_biped(*curorg,fn, novelty);
            }
        }
        //if ES then just dump the evolvability of individuals from this current generation
        else if (NEAT::evolvabilitytest && offspring_count % evolveupdate==0 && evo)
        {
            char fn[100];
            sprintf(fn,"%s_evolvability%d.dat",output_dir,offspring_count/evolveupdate);
            for (curorg = (pop->organisms).begin(); curorg != pop->organisms.end(); ++curorg)
                evolvability_biped(*curorg,fn, false);
        }
        //to quantify evolvability of fitness-based search organisms
        else if (NEAT::evolvabilitytest && offspring_count % evolveupdate==0)
        {
            char fn[100];
            sprintf(fn,"%s_evolvability%d.dat",output_dir,offspring_count/evolveupdate);
            for (curorg = (pop->organisms).begin(); curorg != pop->organisms.end(); ++curorg)
                  evolvability_biped(*curorg,fn, true);
        }
...
mx=(*curorg)->noveltypoint->fitness; b=(*curorg); }
  ...
  //evolvability_biped(b,"dummy");
        ...
        */
        ...
        //calculate the evolvability of the new org
        if (evo)
        {
        	evo_biped(new_org);
        }
    ...
    //printing the evolvability of end-of-run individuals
    if (NEAT::evolvabilitytest && novelty) {
        char fn[100];
        sprintf(fn,"%s_evolvability%s.dat",output_dir,"_end_of_run");
        for (curorg = (pop->organisms).begin(); curorg != pop->organisms.end(); ++curorg)
            evolvability_biped(*curorg,fn, novelty);
    }
    //in case of ES just dumb the evolvability of end-of-run organisms
    else if (NEAT::evolvabilitytest && evo)
    {
        char fn[100];
        sprintf(fn,"%s_evolvability%s.dat",output_dir,"_end_of_run");
        for (curorg = (pop->organisms).begin(); curorg != pop->organisms.end(); ++curorg)
    ...
    //to quantify evolvability of fitness-based search organisms from the end-of-run generation
  else
   {
		char fn[100];
		sprintf(fn,"%s_evolvability%s.dat",output_dir,"_end_of_run");
...
#define BDIM 30
void evolvability_biped(Organism* org,char* fn,bool recall) {
    fstream file;
...
// file << "---" <<  " " << org->winner << endl;
    if (recall)
    {
		...
		for (int i=0; i<BIPEDMUTATIONS; i++) {
	        ...
	        //noveltyitem* nov_item = biped_evaluate_paralell(new_org,NULL);
	        //new_org->noveltypoint=nov_item;
	        offs.push_back(new_org);
	        //delete new_gene;
		}
	    //calling mpi master
	    evall.eval(offs,0,offs.size());
	    for (int i=0; i<BIPEDMUTATIONS; i++){
	   ...
	   //cout<<"max perormance: "<<max_per<<"\n";
	    	...
	    	//cout<<"Is this ok?? "<<offs[i]->noveltypoint->data[0].size()<<endl;
	    for(int k=0;k<offs[i]->noveltypoint->data[0].size();k++)
	       points[i*BDIM+k]=offs[i]->noveltypoint->data[0][k]/25.0;
	   }

	    int dist_B = distinct(points,BIPEDMUTATIONS,BDIM);
            double dist_F = bin_metric(offs,BIPEDMUTATIONS); //distnict_per(offs, BIPEDMUTATIONS, index);
		file << org->gnome->genome_id << " " << dist_B << " " << dist_F << endl;
		//delete offspring
		for (int i=0; i<BIPEDMUTATIONS; i++)
		{
			delete offs[i];
		...
		offs.clear();
			...
			file << org->gnome->genome_id << " " << org->BDO << " " << org->fitness << endl;

		file.close();
}
#define BIPEDMUTATIONS 200
#define BDIM 30
void evo_biped(Organism* org)
{
	vector<Organism*> offs;
	double max_per = DBL_MIN;
	int index = 0;
   ...
   /* float minx=-10.0,maxx=10.0,miny=-10.0,maxy=10.0;
    ...
    int connections*/;
    //data_record rec;
    //cout<<"before spawing offspring the parent org fitness is "<<org->fitness<<endl;
    for (int i=0; i<BIPEDMUTATIONS; i++) {
        Genome *new_gene= new Genome(*org->gnome);
        //new_org->gnome = new Genome(*org->gnome);
        mutate_genome(new_gene);
    ...
    //calling mpi master
    evall.eval(offs,0,offs.size());
    //master resumes evolution
    //cout<<"Master resumes evolution \n";
  	//cout<<"Is this ok?? \n";
  	//cout<<offs[0]->noveltypoint->data[0].size()<<endl;
   ...
   //cout<<"max perormance: "<<max_per<<"\n";
    ...
    for(int k=0;k<offs[i]->noveltypoint->data[0].size();k++){
    	//cout<<" "<<offs[i]->noveltypoint->data[0][k]<<" "<<endl;
       points[i*BDIM+k]=offs[i]->noveltypoint->data[0][k]/25.0;
    }
   }
...
//org->fitness = distnict_per(offs, BIPEDMUTATIONS, index); // if you want to set evolbability based on variation in performance uncomment this line
org->fitness=bin_metric( offs,BIPEDMUTATIONS);
org->BDO=dist;
org->FDO=org->fitness;
cout <<"Evo based on variation in offspring fitness : "<<org->BDO<<" "<<org->fitness<<"\n";
//cout <<"Evo based on variation in offspring Behavior : "<<org<<"\n";
for (int i=0; i<BIPEDMUTATIONS; i++)
{
	delete offs[i];
}
...
//Spawn the Population
    ...
    pop=new Population(start_genome,NEAT::pop_size);
    cout<<"population size is :"<<pop->organisms.size()<<endl;
    cout<<"Verifying Spawned Pop"<<endl;
    pop->verify();
...
int biped_generational_epoch(Population **pop2,int generation,data_rec& Record, noveltyarchive& archive, bool novelty) {
    ...
    if (NEAT::multiobjective) {  
      if (novelty)
        {
            archive.evaluate_population(pop->organisms, true);
           //then add to the archive (maybe remove & only add randomly?)
           archive.evaluate_population(pop,false); 
        }
	for (size_t i = 0; i < pop->organisms.size(); ++i){
         evo_biped(pop->organisms[i]);       
         } 
        //merge and filter population
        for (curorg=(pop->organisms).begin(); curorg!=(pop->organisms).end(); ++curorg) {
...
//evaluate this 'super-population'
        ...
        rank_multi(measure_pop);
        
  for(int i=0;i<measure_pop.size();i++) {
  cout << measure_pop[i]->novelty << " " << measure_pop[i]->FDO << " " <<  measure_pop[i]->noveltypoint->rank << endl;
        }
...
//writing out stuff
    if (generation%NEAT::print_every == 0 )
    {
        ...
        //sprintf(filename,"%s_record.dat",output_dir);
         sprintf(filename,"%s_fittest_%d.dat",output_dir,generation/NEAT::print_every);
        char fname[100];
        sprintf(fname,"%s_archive.dat",output_dir);
        archive.Serialize(fname);
//Record.serialize(filename);
...
void rank_multi(vector<Organism*>&organisms)
{
	 int sz=organisms.size();
	 //reset vars
	 for(int i=0;i<sz;i++) {
		 organisms[i]->noveltypoint->dominationcount=0;
		 organisms[i]->noveltypoint->dominationList.clear();
	 }

	 //do domination
	 for(int i=0;i<sz;i++){
	  for(int j=0;j<sz;j++)
		if(  organisms[i]->dominate(organisms[j])){
			organisms[j]->noveltypoint->dominationcount++;
	        organisms[i]->noveltypoint->dominationList.push_back(organisms[j]->noveltypoint);
	   }
     }
	 ...
	 while(ranked!=sz) {
	  for(int i=0;i<sz;i++)
	  {
		if(organisms[i]->noveltypoint->dominationcount==0) {
		  front.push_back(organisms[i]->noveltypoint);
		  organisms[i]->noveltypoint->dominationcount = -1;
		  organisms[i]->noveltypoint->rank=cur_rank;
		  organisms[i]->fit=sz-cur_rank;
		  ranked++;
		}
	  }
	 ...
	 std::sort(organisms.begin(), organisms.end(), order_orgs_NSGA);
}
