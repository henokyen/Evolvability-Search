...
void evolvability_based(Organism* org);
void Evo_paralell(Organism* org);
void evolvability_org(Organism* org);
using namespace std;
...
Population *maze_novelty_realtime(char* outputdir,const char* mazefile,int par,const char* genes,bool novelty,bool evo) {
    ...
    if (!seed_mode)
    {
        pop=new Population(start_genome,NEAT::pop_size);//Henok: this might be the place to spawn the map-elite
        cout<<"Population size:"<<NEAT::pop_size<< "\n";
	}
/*    else
    {
    ...
    else // Henok: clone from a seed gene to create a population
    {
    	cout<<"Cloning a population from seed: "<<seed_name<<"\n"   ;
    	pop=new Population(start_genome,NEAT::pop_size, 0);
    	cout<<"Population size:"<<NEAT::pop_size<< "\n";
    }
    ...
    maze_novelty_realtime_loop(pop,novelty, evo);
...
int maze_novelty_realtime_loop(Population *pop,bool novelty, bool evo) {
...
//activity stat log file
    ...
    if (evo)
    {
    	//Evo_paralell(pop->organisms);//MPI and TBB
    	//assign fitness scores based on evolvability
    	for (size_t i = 0; i < pop->organisms.size(); ++i){
    		//evolvability_based(pop->organisms[i]);
    		//cout<<"assign fitness scores based on evolvability of org: "  <<i<<"\n";
    		Evo_paralell(pop->organisms[i]);
    	}
...
mx=(*curorg)->noveltypoint->fitness; b=(*curorg); }
        ...
        */
        ...
        //calculate evolvability of the new individual
        if (evo)
        {
        	//cout<<"calculate the evolvability of the new individual"<<"\n";//Henok
        	Evo_paralell(new_org);
        	//evolvability_based(new_org);
        }
        ...
        if (newrec->ToRec[3]>=envList.size() && newrec->ToRec[4]>=envList.size()) {
            if (false) //NEAT::evolvabilitytest)// Henok: you may uncomment this to see evolvability of solutions
            {
...
//disable quit for now
        ...
        if (!firstflag && (newrec->ToRec[3]>=envList.size() && newrec->ToRec[4]>=envList.size())) {
		  ...
		  sprintf(eval_filename,"%sEval#",output_dir);
		  std::ofstream outFile(eval_filename,std::ios::out);
		  outFile<<"Evaluations: "<<offspring_count<<std::endl<<std::endl;
		  outFile.close();
		  //if
		  if ((new_org)->winner)
		  {
			cout<<"Writting the soluton to a file for adaptability test \n";
			char soln[100];
			sprintf(soln,"%soln",output_dir);
			std::ofstream outFilesoln(soln,std::ios::out);
			((new_org)->gnome)->print_to_file(outFilesoln);
			outFilesoln.close();
		  }
        break;
        }
...
double mazesim(Network* net, vector< vector<float> > &dc, data_record *record,Environment* the_env,Organism* o=NULL)
{
    ...
    //if o is offspring then simulate it only for 200 time steps
    if (o->off)
    {
    	//cout<<"running a child for 200 time steps \n";
    	timesteps=300;
    }
    ...
    if (novelty_measure == novelty_sample ||
            novelty_measure ==novelty_sample_free)
        data.reserve(2); //timesteps/stepsize);
    ...
    if (fitness_measure == fitness_goal)
    {
        ...
        if (newenv->reachgoal && o!=NULL) o->winner=true;
        else o->winner=false;   
    //if (newenv->hero.collide)
        //	fitness+=50;
    }
...
// file << "---" <<  " " << org->winner << endl;
    ...
    for (int i=0; i<MUTATIONS; i++) {
        ...
        noveltyitem* nov_item = maze_novelty_map(new_org, NULL);
          cout<<"Fitness: "<<nov_item->fitness<<"\n";
        if (i==0) {
        ...
        if (new_org->winner) solution=true;
        if(recall) {
        ...
        cout<<nov_item->data[0][0]<<" "<<(nov_item->data[0][0]-minx)/(maxx-minx) <<"\n";
        points[i*2+1]=(nov_item->data[0][1]-miny)/(maxy-miny);
        cout<<nov_item->data[0][1]<<" "<<(nov_item->data[0][1]-miny)/(maxy-miny) <<"\n";
        delete new_org;
        delete nov_item;
        //file << endl;
    }
...
//    fstream file;
//    file.open(fn,ios::app|ios::out);
    //cout <<"Measuring Evolvability..." << endl;
// file << "---" <<  " " << org->winner << endl;
    double points[MUTATIONS*2];
//    double performance [MUTATIONS];
//    double max_per = DBL_MIN;
//    int index = 0;
    float minx,maxx,miny,maxy;
    envList[0]->get_range(minx,miny,maxx,maxy);// i think due to this line the function can't be parallelized
...
//    int connections;
    for (int i=0; i<MUTATIONS; i++)
    {
    	//cout<<org->noveltypoint->fitness;
    	Genome *new_gene= new Genome(*org->gnome);
//
        //new_org->gnome = new Genome(*org->gnome);
//        if (i!=0) //first copy is clean
//            for (int j=0; j<1; j++)
        mutate_genome(new_gene);
        Organism *new_org= new Organism(0.0,new_gene,0);

        noveltyitem* nov_item = maze_novelty_map(new_org,NULL);
         //org->noveltypoint= maze_novelty_map(org);
       //cout<<"Fitness: "<<nov_item->fitness<<"\n";

        //Henok I think all indis have been evaluated at this point, so no need to do it again
//        if (i==0) {
//            fit=nov_item->fitness;
//            //org->soln = new_org->winner;
//            //cout<<org->soln<<" "<<fit<<"\n";
//            nodes=new_org->net->nodecount();
//            connections=new_org->net->linkcount();
//            //cout << fit << endl;
...
//        if (new_org->winner)
//        	solution=true;
        // Accumulating the performance of offspring
//        performance[i] = nov_item->fitness;
//        //cout<<"perormance: "<<performace[i]<<"\n";
//        if (performance[i] >max_per)
//        {
//        	max_per = performance[i];
//        	index = i;
//        	//cout<<"max perormance: "<<max_per<<"\n";
//        }
        points[i*2]=(nov_item->data[0][0]-minx)/(maxx-minx);
        //cout<<nov_item->data[0][0]<<" "<<(nov_item->data[0][0]-minx)/(maxx-minx) <<"\n";
       points[i*2+1]=(nov_item->data[0][1]-miny)/(maxy-miny);
        //cout<<nov_item->data[0][1]<<" "<<(nov_item->data[0][1]-miny)/(maxy-miny) <<"\n";
        //cout <<"around ending point ..." << endl;
        delete new_org;
        delete nov_item;
    }
    int dist = distinct(points,MUTATIONS,2);
//    //now fitness is based on evolvability
//    double dist = distnict_per(performance, MUTATIONS, index); // if you want to set evolbability based on variation in performance uncomment this line
//    cout <<"Evo based on variation in performance: "<<dist<<"\n";
    org->fitness=dist;
    //org->evolvability = dist;
    //std::cout<<"evolvability: "<<dist<<"\n";
    //std::cout<<org->gnome->traits[0]->params[0]<< " "<<org->gnome->traits[0]->params[1]<<"\n";
    //org->set_desc(org->gnome->traits[0]->params[0],org->gnome->traits[0]->params[1]);
    }
void evolvability_org(Organism* org)
{
	org->noveltypoint=maze_novelty_map(org, NULL);
...
public:
	typedef std::vector<type*>org_t;
	org_t _org;
	type * _par;
	Evo(org_t & org, type * p) : _org(org),_par(p){}
	void operator()(sferes::parallel::range_t& r ) const {
	for (size_t i=r.begin(); i!=r.end(); ++i ) {
		//cout<<i<<" "<<_org[i]->winner<<"\n";
		evolvability_org(_org[i]);
		//evolvability_based_parent(_org[i], _par);
	 }
	}
};
#define MUTATIONS 200
void Evo_paralell(Organism* org)
{
   vector<Organism*> offs;
   Organism *new_org;
   Genome *new_gene;

   //Henok preparing the 200 offspring
   for (int i=0; i<MUTATIONS; i++){
	   new_gene= new Genome(*org->gnome);
	   mutate_genome(new_gene);
	   new_org= new Organism(0.0,new_gene,0);
	   new_org->off=false;
	   offs.push_back(new_org);
   }
  //Henok send the offspring for evaluation
   sferes::parallel::init();
   sferes::parallel::p_for(sferes::parallel::range_t(0, offs.size()),
     		Evo<Organism>(offs, org));
...
// file << "---" <<  " " << org->winner << endl;
  ...
  //double performance [MUTATIONS];
   ...
   //int connections;
   for (int i=0; i<MUTATIONS; i++){
   //performance[i] = offs[i]->fitness;
   //cout<<"perormance: "<<performace[i]<<"\n";
    if (offs[i]->fitness >max_per)
     {
	max_per =offs[i]->fitness;
	index = i;
//cout<<"max perormance: "<<max_per<<"\n";
     }
  //points[i*2]=(offs[i]->noveltypoint->data[0][0]-minx)/(maxx-minx);
   //cout<<"nov_item->data[0][0]: "<<offs[i]->noveltypoint->data[0][0] <<"\n";
  //points[i*2+1]=(offs[i]->noveltypoint->data[0][1]-miny)/(maxy-miny);
   //cout<<"nov_item->data[0][1]: "<<offs[i]->noveltypoint->data[0][1] <<"\n";
   //cout <<"around ending point ..." << endl;
  }
//int dist = distinct(points,MUTATIONS,2);
//now fitness is based on evolvability
double dist = distnict_per(offs, MUTATIONS, index); // if you want to set evolbability based on variation in performance uncomment this line
//double dist = distnict_per_parent(offs, MUTATIONS, index);
//double dist =  performace_std(offs,MUTATIONS);
org->fitness=dist;
//cout <<"Evo based on variation in distance: "<<org->fitness<<"\n";
//clear the offspring
for (int i=0; i<MUTATIONS; i++){
	delete offs[i];
}
